...
NAME="protect-main"
# Idempotent create/update: delete if exists with same name, then create.
EXIST_ID=$(gh api "orgs/$ORG_NAME/rulesets" --jq ".[] | select(.name==\"$NAME\") | .id" || true)
[ -n "$EXIST_ID" ] && gh api -X DELETE "orgs/$ORG_NAME/rulesets/$EXIST_ID" || true
